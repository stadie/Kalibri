...
void cmsPrel(double intLumi=-1, bool wide = false, long int sqrtS = 7) {
  ...
  if (intLumi > 0.) {
    ...
    Form("CMS preliminary, %.1f fb^{-1}",intLumi));
  }
  else if (intLumi==0) { // simulation
    latex->SetTextAlign(11); // align left
    latex->DrawLatex(wide ? 0.06 : wideLeftOffset, 0.96, "CMS Preliminary");
  }
  else {
    latex->SetTextAlign(11); // align left
...
void cmsWIP(bool wide = false, long int sqrtS = 8) {
...
latex2->DrawLatex(wide ? 0.98 : 0.95, 0.96, ((TString)"#sqrt{s} = "+ sqrtS)+ " TeV");
  //}
} // cmsWIP
