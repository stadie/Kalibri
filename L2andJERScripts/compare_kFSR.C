...
void compare_kFSR(TString dir1, TString dir2, TString algo="PF",
		  TString dir_prefix="KOSTAS_L1_on_pt_plain_", TString label_dir1="", TString label_dir2="",TString binning_select ="kostas", Bool_t pub_style=true){
  setTDRStyle();
  ...
  Double_t kFSR_extrapol_ymax=1.1;
    ...
    //"kFSR_comp_"+
    cmsPrel(intLumi=36, false);
    c->SaveAs(GetDateDir()+"/kFSR_comp_"+dir1+"_kFSR_"+ algo +".eps");

    import_kFSR_vs_Abseta_histo_res1_Dir2->Draw("e");

    import_kFSR_vs_Abseta_histo_res1_Dir2->Draw("hist e");
    ...
    r = import_kFSR_vs_Abseta_histo_res1_Dir2->Fit(kFSR_fit,"S N 0");
    ...
    line->Draw();
    cmsPrel(intLumi=36, false);
    c->SaveAs(GetDateDir()+"/kFSR_comp_"+dir2+"_kFSR_"+ algo +".eps");
    ...
    leg_kFSR_comb = new TLegend(0.25,0.70,0.65,0.85);
    //    leg_kFSR_comb->SetFillColor(kWhite);
    leg_kFSR_comb->SetFillStyle(kNone);
    leg_kFSR_comb->SetTextFont(42);
      //   leg->SetHeader("Legende");
    if(pub_style){
    leg_kFSR_comb->AddEntry(import_kFSR_vs_Abseta_histo_res1_Dir1,"2010","lp");
    leg_kFSR_comb->AddEntry(import_kFSR_vs_Abseta_histo_res1_Dir2,"2011","lp");
    }
    else{
...
//    leg_kFSR_comb->AddEntry(band2,"Fit uncertainty ("+dir2+")","f");
    ...
    c->SaveAs(GetDateDir()+"/kFSR_comp_"+dir1+"_"+dir2+"_kFSR_"+ algo +".eps");
