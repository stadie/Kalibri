...
TH1D* get_absolute_uncertainty_band(TH1D* draw_hist, std::vector<TF1*> functions_, std::vector<TH1D*> hist_over_denomHist){

  Int_t no_x_bins = 100;
  //  cout << "works here..INFUNC" << functions_.size() <<endl;

  TH1D* abs_uncertainty_band = new TH1D("abs_uncertainty_band", "", no_x_bins, draw_hist->GetXaxis()->GetXmin(), draw_hist->GetXaxis()->GetXmax());
  //  cout << "works here..INFUNC" << functions_.size() <<endl;
    ...
    for(Int_t i=1;i< no_x_bins+1;i++){
      ...
      //      cout << "works here..INFUNC" << functions_.size()  <<BinCenter << endl;
      Double_t sum_x2 = 0;
      //      cout << "works here..BINCENTER: " << BinCenter  << ", "<< sum_x2 << ", " << functions_.at(0)->Eval(BinCenter) << endl;
      for(unsigned int f_i=0;f_i<functions_.size();f_i++){
	//	cout << "test" << endl;
	//	cout << functions_.at(f_i)->Eval(BinCenter) << endl;
	//	cout << TMath::Power(functions_.at(f_i)->Eval(BinCenter),2) << endl;
	//	cout << TMath::Abs(TMath::Power(functions_.at(f_i)->Eval(BinCenter),2)) << endl;
	if(functions_.at(f_i)->GetName()!="NOFIT")sum_x2+= TMath::Abs(TMath::Power(functions_.at(f_i)->Eval(BinCenter),2));
	//	cout << "works here..BINCENTER: " << BinCenter  << ", "<< sum_x2 <<endl;
      }
...
TCanvas* plotRatios_wFit(TH1D* numeratorHist, TH1D* denominatorHist, TString title="", TString xTitle="", TString yTitle="") {  
...
TCanvas* plotRatios_wFit(TH1D* numeratorHist1, TH1D* numeratorHist2, TH1D* numeratorHist3, TH1D* denominatorHist, TString title="", TString xTitle="", TString yTitle="") {
  std::vector<TH1D*> numeratorHistograms;
  numeratorHistograms.push_back( numeratorHist1 );
  numeratorHistograms.push_back( numeratorHist2 );
