...
def BinningValues(BINNING,AbsEta):
    if(BINNING=="kostas"):
        ...
        binning_values = "-5.191 -3.489 -3.139 -2.964 -2.853 -2.5 -2.322 -2.172 -2.043 -1.930 -1.830 -1.740 -1.653 -1.566 -1.479 -1.392 -1.305 -1.218 -1.131 -1.044 -0.957 -0.879 -0.783 -0.696 -0.609 -0.522 -0.435 -0.348 -0.261 -0.174 -0.087 0.000 0.087 0.174 0.261 0.348 0.435 0.522 0.609 0.696 0.783 0.879 0.957 1.044 1.131 1.218 1.305 1.392 1.479 1.566 1.653 1.740 1.830 1.930 2.043 2.172 2.322 2.5 2.853 2.964 3.139 3.489 5.191"
        abs_binning_values = "0.000 0.087 0.174 0.261 0.348 0.435 0.522 0.609 0.696 0.783 0.879 0.957 1.044 1.131 1.218 1.305 1.392 1.479 1.566 1.653 1.740 1.830 1.930 2.043 2.172 2.322 2.5 2.853 2.964 3.139 3.489 5.191"    
...
#        abs_binning_values = "0 0.783 1.305 1.93 2.5 2.964 3.2 5.191"
    ...
    elif(BINNING=="CaloTower"):
        binning_values = "-5.191 -4.889 -4.716 -4.538 -4.363 -4.191 -4.013 -3.839 -3.664 -3.489 -3.314 -3.139 -2.964 -2.853 -2.650 -2.500 -2.322 -2.172 -2.043 -1.930 -1.830 -1.740 -1.653 -1.566 -1.479 -1.392 -1.305 -1.218 -1.131 -1.044 -0.957 -0.879 -0.783 -0.696 -0.609 -0.522 -0.435 -0.348 -0.261 -0.174 -0.087 0.000 0.087 0.174 0.261 0.348 0.435 0.522 0.609 0.696 0.783 0.879 0.957 1.044 1.131 1.218 1.305 1.392 1.479 1.566 1.653 1.740 1.830 1.930 2.043 2.172 2.322 2.500 2.650 2.853 2.964 3.139 3.314 3.489 3.664 3.839 4.013 4.191 4.363 4.538 4.716 4.889 5.191"
        abs_binning_values = "0.000 0.087 0.174 0.261 0.348 0.435 0.522 0.609 0.696 0.783 0.879 0.957 1.044 1.131 1.218 1.305 1.392 1.479 1.566 1.653 1.740 1.830 1.930 2.043 2.172 2.322 2.500 2.650 2.853 2.964 3.139 3.314 3.489 3.664 3.839 4.013 4.191 4.363 4.538 4.716 4.889 5.191"
    else:
...
#            fcfg.write("Use single jet triggers = true\n")
        ...
        if(DATAYEAR == "2013" and USE_NEW_TRIGGERS_AND_FASTPF==1):
            trigger_names = "Di-Jet trigger names = HLT_DiPFJetAve40;HLT_DiPFJetAve80;HLT_DiPFJetAve140;HLT_DiPFJetAve200;HLT_DiPFJetAve260;HLT_DiPFJetAve320;HLT_DiPFJetAve400\n"
            #Denis June 2012 on 3fb^-1 @8TeV
            if(jettype == "ak5PF"):
                trigger_thresholds = "60 105 174 242 311 380 468 \n"
...
#        PU_weighting_info = "PU weighting era = Summer12\n PU weighting histogram = /scratch/hh/current/cms/user/kirschen/PUDistributions/Inclusive/MyDataPileupHistogramObservedAllHLT.root \n PU TruthWeighting = kirschen/PUDistributions/Cert_2012_190456-193336 \n PU TruthWeighting MC distribution = TrueSummer12 \n"
    ...
    if(MC_type=="44Z2wPU" or MC_type=="44Z2wPUsmY0612" or MC_type=="44Z2wPUsmY0612_u" or MC_type=="44Z2wPU_PUUP"):
        ...
        PU_weighting_info = PU_weighting_info + "PU TruthWeighting MC distribution = rathjd/TrueDistributions/Summer12S10CMSSW53 \n"
    else:
...
def determineDataDir(DATAYEAR,DATATYPE):
    ...
    if(DATAYEAR == "2013"):
	    ...
	    datadir = "/nfs/dust/test/cms/user/rathjd/Summer13ReReco_v1" 	       
    if(DATAYEAR == "MC2012"):
        ...
        if(DATATYPE=="Z253"):
            datadir = "/nfs/dust/test/cms/user/rathjd/Summer12_DR53X-PU_S10_START53_V7A-v1/"	  
...
def determineDataDirMC(MC,MC_type):
    if(MC == "Su12"):
        ...
        if(MC_type=="Z253"):
            datadirmc = "/nfs/dust/test/cms/user/rathjd/Summer12_DR53X-PU_S10_START53_V7A-v1/"	  
